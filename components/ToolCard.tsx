      ...
      <div className="flex items-start justify-between gap-3">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1.5 flex-wrap">
            <span className="text-base font-bold" style={{ fontFamily: "var(--font-space)", color: "var(--text)" }}>
              {tool.name}
            </span>
            {tool.tag && tool.tagType && (
              <span className="text-[11px] font-semibold px-2 py-0.5 rounded-full whitespace-nowrap"
                style={{ background: tagColors[tool.tagType]?.bg, color: tagColors[tool.tagType]?.color }}>
                {tool.tag}
              </span>
            )}
            {tool.badge && (
          ...
          <p className="text-[13.5px] leading-snug m-0" style={{ color: "var(--text2)" }}>{tool.desc}</p>
        </div>

        {/* Upvote button */}
        <button
          onClick={handleUpvote}
          className="flex flex-col items-center gap-0.5 px-2.5 py-2 rounded-xl flex-shrink-0 transition-all duration-150 cursor-pointer"
          style={{
      ...
      <div className="flex items-center justify-between pt-3 flex-wrap gap-2" style={{ borderTop: "1px solid var(--border)" }}>
        <div className="flex items-center gap-3.5">
          <div className="flex items-center gap-1">
            ...
            <span className="text-xs" style={{ color: "var(--text3)" }}>({tool.reviews})</span>
          </div>
